...
export default function Cart({ position, closeCart }) {
  ...
  return (
    ...
    >
      <button
        className="text-zinc-50 w-full flex items-center py-2 px-5 gap-5 bg-emerald-600"
        onClick={closeCart}
      >
        <svg
      ...
      {cartItems.length > 0 && (
        <Link
          to={"/cartpage"}
          className="py-3 px-5 text-1xl font-bold text-zinc-50 text-center bg-green-500"
        >
          Finalizar compra <span>R$ {totalPrice().toFixed(2)}</span>
        </Link>
      )}
    </div>
  );
